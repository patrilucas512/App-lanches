...
if (!(Test-Path -LiteralPath $ConfigPath)) { throw "Configuração do Conector Mesa Viva não encontrada." }
  ...
  public static void Send(string printerName, byte[] bytes) {
    ...
    try {
      ...
      try {
        ...
        try {
          int written;
          if (!WritePrinter(printer, bytes, bytes.Length, out written) || written != bytes.Length) throw new System.ComponentModel.Win32Exception(Marshal.GetLastWin32Error());
...
function Send-Receipt([string]$Text, [string]$PrinterName) {
  $clean = Convert-ToPrinterText $Text
  $encoding = [Text.Encoding]::GetEncoding(850)
  $content = $encoding.GetBytes($clean + "`r`n`r`n`r`n")
  $bytes = New-Object 'System.Collections.Generic.List[byte]'
  $bytes.AddRange([byte[]](27,64))
  $bytes.AddRange($content)
  $bytes.AddRange([byte[]](29,86,66,0))
...
function Invoke-MesaVivaRpc([string]$FunctionName, [hashtable]$Body) {
  ...
  $uri = "$($config.SupabaseUrl.TrimEnd('/'))/rest/v1/rpc/$FunctionName"
  return Invoke-RestMethod -Method Post -Uri $uri -Headers $headers -ContentType "application/json" -Body ($Body | ConvertTo-Json -Compress)
}
...
while ($true) {
  try {
    ...
    foreach ($job in @($response.jobs)) {
      try {
        Send-Receipt ([string]$job.text) ([string]$response.printer_name)
    ...
    Write-ConnectorLog "Falha de conexão: $($_.Exception.Message)"
  }
