alter table public.service_modes
  add column if not exists printer_connection text not null default 'usb',
  add column if not exists printer_name text,
  add column if not exists printer_paper_width integer not null default 58,
  add column if not exists printer_network_address text,
  add column if not exists printer_setup_completed boolean not null default false;

alter table public.service_modes drop constraint if exists service_modes_printer_connection_check;
alter table public.service_modes add constraint service_modes_printer_connection_check check (printer_connection in ('usb','bluetooth','network'));
alter table public.service_modes drop constraint if exists service_modes_printer_paper_width_check;
alter table public.service_modes add constraint service_modes_printer_paper_width_check check (printer_paper_width in (58,80));
