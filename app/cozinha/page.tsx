...
export default async function KitchenPage() {
  ...
  const [{ data: tickets }, { data: tableOrders }, { data: publicOrders }, { data: sessions }, { data: tables }, { data: tableItems }, { data: publicItems }, { data: serviceMode }, { data: directPrinter }, { data: businessHours }] = await Promise.all([
  ...
  return <KitchenBoard establishmentId={establishment.id} establishmentName={establishment.name} operatorName={operator?.name || (member.role === "owner" ? "Administrador" : "Gerente")} canReturnToAdmin={member.role === "owner" || member.role === "manager"} canPrint={member.role !== "kitchen" || operator?.permissions?.print_orders !== false} autoPrint={serviceMode?.auto_print_kitchen ?? false} paperWidth={serviceMode?.printer_paper_width === 80 ? 80 : 58} directPrinter={(directPrinter || { configured: false, online: false }) as never} businessHours={businessHours || []} initial={{ tickets: tickets || [], tableOrders: tableOrders || [], publicOrders: publicOrders || [], sessions: sessions || [], tables: tables || [], tableItems: tableItems || [], publicItems: publicItems || [] }} />;
}
