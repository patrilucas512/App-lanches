...
export default async function CashierPage() {
  ...
  const [{ data: employees }, { data: movements }, { data: payments }, { data: shifts }, { data: products }] = await Promise.all([
    ...
    supabase.from("cash_shifts").select("*").eq("establishment_id", establishment.id).order("opened_at", { ascending: false }).limit(100),
    supabase.from("products").select("id,name,barcode,price_cents").eq("establishment_id", establishment.id).order("name"),
  ]);
  return <DashboardShell active="cashier" storeSlug={establishment.slug} role={member.role}><header className="dashboard-head"><div><small>FINANCEIRO E RESPONSABILIDADE</small><h1>Caixa.</h1><p>PDV completo, equipe, recebimentos, código de barras e fechamento identificado.</p></div><span className="status-pill">Controle ativo</span></header><CashierAdmin initialShifts={(shifts || []) as never[]} initialProducts={(products || []) as never[]} /><CashierManager establishmentId={establishment.id} userId={userId} employees={(employees || []) as never[]} initialMovements={(movements || []) as never[]} tablePayments={(payments || []) as never[]} /></DashboardShell>;
}
