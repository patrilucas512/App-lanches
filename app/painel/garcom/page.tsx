...
export default async function WaiterPage() {
  ...
  const [{ data: tables }, { data: sessions }, { data: products }, { data: categories }, { data: serviceMode }, { data: waiter }, { data: tickets }, { data: orders }, { data: publicOrders }, { data: profile }] = await Promise.all([
    ...
    supabase.from("waiters").select("id,name,status,active_now,permissions,employment_type,work_date").eq("establishment_id", establishment.id).eq("user_id", userId).maybeSingle(),
    ...
    supabase.from("orders").select("id,restaurant_table_id,order_number,fulfillment_type,created_at").eq("establishment_id", establishment.id).order("created_at", { ascending: false }).limit(100),
  ...
  return <DashboardShell active="waiter" storeSlug={establishment.slug} role={member.role}>
    ...
    }} /> : <section className="panel waiter-mode-off"><b>Modo garçom desativado</b><p>Os pedidos diretos continuam somente na cozinha e no balcão. Ative o modo garçom em Atendimento para liberar esta operação.</p></section>}
    <WaiterConsole establishmentId={establishment.id} establishmentName={establishment.name} userId={userId}
      operatorName={waiter?.name || profile?.full_name || "Administrador"}
