...
export default async function WaiterPage() {
  ...
  const [{ data: tables }, { data: sessions }, { data: products }, { data: categories }, { data: serviceMode }, { data: waiter }] = await Promise.all([
    ...
    supabase.from("waiters").select("id,status,active_now,permissions").eq("establishment_id", establishment.id).eq("user_id", userId).maybeSingle(),
  ]);
  if (member.role === "attendant" && (!serviceMode?.waiter_mode_enabled || !waiter?.active_now || ["inactive", "paused", "blocked"].includes(waiter?.status || ""))) redirect("/garcom/login");
