...
export default async function DashboardPage() {
  ...
  const [{ data: orders }, { count: products }, { data: subscription }, { data: events }] = await Promise.all([
    ...
    supabase.from("usage_events").select("event_type").eq("establishment_id", establishment.id).gte("created_at", today.toISOString()),
  ]);
  const revenue = (orders ?? []).reduce((total, order) => total + order.total_cents, 0);
  const views = events?.filter(event => event.event_type === "menu_view").length ?? 0;
  const whatsappOrders = events?.filter(event => event.event_type === "order_whatsapp").length ?? 0;
  const conversion = views ? Math.round((whatsappOrders / views) * 100) : 0;
  return <DashboardShell storeSlug={establishment.slug}>
    ...
    <div className="dashboard-cards">
      ...
      <article className="dash-card"><small>VISITAS NO CARDÁPIO</small><strong>{views}</strong></article>
      <article className="dash-card"><small>CONVERSÃO WHATSAPP</small><strong>{conversion}%</strong></article>
    </div>
