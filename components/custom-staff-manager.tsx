...
export function CustomStaffManager({ establishmentId, initialRoles }: { establishmentId: string; initialRoles: StaffRole[] }) {
  ...
  async function saveRole(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const values = { establishment_id: establishmentId, name: String(form.get("name")).trim(), description: String(form.get("description") || "").trim() || null, color: String(form.get("color") || "#7b2326"), ...(imageUrl ? { image_url: imageUrl } : {}) };
      ...
      const { data, error } = await query.select("id,name,description,color,image_url").single();
      if (error) throw error;
  ...
  return <section className="custom-staff-area">
    ...
    {roles.map(role => <article className="panel custom-role-card" key={role.id} style={{ borderTopColor: role.color }}>
      <header className={`custom-role-header ${role.image_url ? "has-image" : ""}`} style={role.image_url ? { backgroundImage: `linear-gradient(90deg, rgba(20,20,16,.92), rgba(20,20,16,.55)), url(${role.image_url})` } : { backgroundColor: `${role.color}18` }}>
        <div><small>FUNÇÃO PERSONALIZADA</small><h2>{role.name}</h2><p>{role.description || "Equipe personalizada pelo estabelecimento."}</p></div>
        <div className="team-heading-actions"><span className="status-pill">{role.custom_staff_members.filter(member => member.status === "active").length} ativos</span><button type="button" className="button dark" onClick={() => setMemberRoleId(current => current === role.id ? null : role.id)}>{memberRoleId === role.id ? "Fechar cadastro" : "Realizar cadastro de funcionário"}</button><button type="button" className="button outline" onClick={() => { setEditingRoleId(role.id); setRoleFormOpen(true); }}>Personalizar</button><button type="button" className="danger-action" onClick={() => void removeRole(role)}>Excluir função</button></div>
      </header>
