...
export function PublicMenuView({ menu, tableNumber, source }: { menu: PublicMenu; tableNumber?: string; source?: string }) {
  ...
  function add(product: MenuProduct) {
    ...
    if (existing) setCart(current => current.map(item => item.key === existing.key ? { ...item, quantity: Math.min(999, item.quantity + 1) } : item));
  ...
  function changeQuantity(key: string, delta: number) { setCart(current => current.map(item => item.key === key ? { ...item, quantity: Math.max(0, Math.min(999, item.quantity + delta)) } : item).filter(item => item.quantity > 0)); }
  function productQuantity(productId: string) { return cart.filter(item => item.product.id === productId).reduce((sum, item) => sum + item.quantity, 0); }
  function removeOneProduct(productId: string) {
    setCart(current => {
      const candidates = current.map((item, index) => ({ item, index })).filter(value => value.item.product.id === productId);
      const target = candidates.find(value => !value.item.addons.length && !value.item.removedIngredients.length) || candidates.at(-1);
      if (!target) return current;
      return current.map((item, index) => index === target.index ? { ...item, quantity: item.quantity - 1 } : item).filter(item => item.quantity > 0);
    });
  }
  function openAddons(product: MenuProduct) {
    const existing = cart.find(item => item.product.id === product.id && !item.removedIngredients.length);
    if (existing) setCustomizingKey(existing.key);
    else {
      const key = crypto.randomUUID();
      setCart(current => [...current, { key, product, quantity: 1, addons: [], removedIngredients: [], unitPrice: product.price_cents }]);
      setCustomizingKey(key); record("product_added", product.id);
    }
    setCheckout(true);
  }
  ...
  return <main className="menu-page" style={{ "--wine": menu.establishment.accent_color, "--paper": menu.establishment.secondary_color ?? "#f5efe5" } as React.CSSProperties}>
    ...
    <section className="menu-content">{menu.categories.filter(category => category.products.length).map(category => <div className="menu-category" key={category.id}><div className="menu-category-title"><h2>{category.name}</h2>{category.description && <p>{category.description}</p>}</div><div className="product-grid">{category.products.map(product => {
      const selectedQuantity = productQuantity(product.id);
      return <article className="product-card" key={product.id}><div className="product-info"><h3>{product.name}</h3>{product.ingredients?.length ? <p className="product-ingredients"><b>Leva:</b> {product.ingredients.join(", ")}</p> : <p>{product.description}</p>}<div className="product-purchase-row"><strong>{money(product.price_cents)}</strong>{service.customer_self_order_enabled && <div className="product-order-actions"><div className={`product-quantity ${selectedQuantity ? "selected" : ""}`}><button type="button" disabled={!selectedQuantity} aria-label={`Diminuir quantidade de ${product.name}`} onClick={() => removeOneProduct(product.id)}>−</button><b>{selectedQuantity}</b><button type="button" aria-label={`Aumentar quantidade de ${product.name}`} onClick={() => add(product)}>+</button></div>{product.addon_options?.length ? <button type="button" className="addons-button" onClick={() => openAddons(product)}>Adicionais</button> : null}</div>}</div></div><div className="product-image" style={product.image_url ? { backgroundImage: `url(${product.image_url})` } : undefined} /></article>;
    })}</div></div>)}</section>
